  ...
  fetch(`https://pokeapi.co/api/v2/pokemon-species/${obj.name}`)
    ...
    .then(function (objTwo) {
      ...
      if (objTwo.evolves_from_species === null) {
        ...
        tdEvolveFrom.innerHTML =
          '<a href="#user_search_bar">' +
          objTwo.evolves_from_species['name'][0].toUpperCase() +
          objTwo.evolves_from_species['name'].substring(1) +
          '</a>';
        if (tdEvolveFrom.innerHTML !== 'None') {
          ...
          clone.addEventListener('click', function handler() {
            clearPage(true);
            fetch(
              `https://pokeapi.co/api/v2/pokemon/${objTwo.evolves_from_species['name']}`
            )
              .then((res) => res.json())
      ...
      fetch(`${objTwo.evolution_chain['url']}`)
        ...
        .then(function (objFour) {
          ...
          if (objFour.chain.evolves_to.length > 0) {
            if (
              objFour.chain.evolves_to.length > 1 ||
              objFour.chain.evolves_to[0].evolves_to.length > 1
            ) {
              ...
              evoChain.forEach((evo) => {
                ...
                innerString +=
                  ', <a id="' +
                  evo.species['name'] +
                  '"href="#user_search_bar">' +
                  evo.species['name'] +
                  '</a>';
              });
              tdEvolveTo.innerHTML = innerString.substring(1);
              for (let i = 0; i < evoChain.length; i++) {
                const tableEvo = document.querySelector(
                  `#${evoChain[i].species['name']}`
                );
                tableEvo.addEventListener('click', function () {
                  clearPage(true);
                  fetch(
                    `https://pokeapi.co/api/v2/pokemon/${evoChain[i].species['name']}`
                  )
                    .then((res) => res.json())
              ...
              objFour.chain.evolves_to.length < 2 &&
              objFour.chain.species['name'] === obj.name.toLowerCase()
            ) {
              tdEvolveTo.innerHTML =
                '<a href="#user_search_bar">' +
                objFour.chain.evolves_to[0].species['name'][0].toUpperCase() +
                objFour.chain.evolves_to[0].species['name'].substring(1) +
                '</a>';
              pokemon = objFour.chain.evolves_to[0].species['name'];
            } else if (
              objFour.chain.evolves_to[0].species['name'] ===
                obj.name.toLowerCase() &&
              objFour.chain.evolves_to[0].evolves_to.length > 0
            ) {
              tdEvolveTo.innerHTML =
                '<a href="#user_search_bar">' +
                objFour.chain.evolves_to[0].evolves_to[0].species[
                  'name'
                ][0].toUpperCase() +
                objFour.chain.evolves_to[0].evolves_to[0].species[
                  'name'
                ...
                '</a>';
              pokemon =
                objFour.chain.evolves_to[0].evolves_to[0].species['name'];
            } else {
              tdEvolveTo.innerHTML = 'None';
            }
          }
...
function createTypeListing(obj, e) {
  ...
  fetch(`https://pokeapi.co/api/v2/type/${searchedType}`)
    ...
    .then(function (objTwo) {
      ...
      type.innerHTML =
        objTwo['name'][0].toUpperCase() + objTwo.name.substring(1);
      ...
      for (let i = 0; i < objTwo.pokemon.length; i++) {
        ...
        fetch(`${objTwo.pokemon[i].pokemon.url}`)
          ...
          .then(function (objThree) {
            ...
            li.innerHTML =
              objTwo.pokemon[i].pokemon.name[0].toUpperCase() +
              objTwo.pokemon[i].pokemon.name.substring(1);
...
function setDamage(damageType, obj) {
  ...
  for (let i = 0; i < obj.damage_relations[damageType].length; i++) {
    innerString += `, ${
      obj.damage_relations[damageType][i].name[0].toUpperCase() +
